...
#include "aes_128_ctr.hpp"

int main(int argc, char* argv[]) {
    std::string input_file, output_file;
    unsigned char key_bytes[16], iv_bytes[16];
    __m128i key, iv;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-in") {
            ...
            key = _mm_loadu_si128(reinterpret_cast<const __m128i*>(key_bytes));
            key_provided = true;
        } else if (arg == "-iv") {
    ...
    AES_128_CTR_ENC aes_ctr(key, iv, firstBlock);
